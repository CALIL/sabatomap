...
createTable = ->
  ...
  for point in POINT_2_LATLNG
    conditions = []
    # nearest1
    for k, v of n1map
      if v[0] is point.lane and v[1] is point.point
        b =
          uuid: ''
          major: 105
          minor: Number(k)
        conditions.push({type: 'neareset1', beacon: b})

    # nearest2
    for d in n2map
      if d[1][0] is point.lane and d[1][1] is point.point
        beacons = [
          {uuid: '', major: 105, minor: d[0][0]}
          {uuid: '', major: 105, minor: d[0][1]}
        ]
        conditions.push({type: 'neareset2', beacon: beacons})
    ...
    for k, v of ndmap
      ...
      if offset?
        if v[0][0] is point.lane and v[0][1] is point.point
          conditions.push({type: 'nearesetD', beacon: b, offset: offset, wide: 90})

        if v[1][0] is point.lane and v[1][1] is point.point
          conditions.push({type: 'nearesetD', beacon: b, offset: (offset + 180) % 360, wide: 90})

    map.push({latlng: point.latlng, conditions: conditions, properties: {point: [point.lane, point.point]}})
  return map
