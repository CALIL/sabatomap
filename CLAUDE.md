...
| 見えたもの | |
|---|---|
| ranging のコールバック | **毎秒来る**（止まらない） |
| その結果 | **常に空** |
| UUID を絞らない領域 | やはり空 |
| `getAuthorizationStatus()` | `AuthorizationStatusAuthorized` |
| 権限（付近のデバイス・位置情報） | どちらも許可済み |
...
### ★ 最初の startRangingBeacons は取りこぼされる。張り直すときは stop を挟む

autobind は**最初の `startRangingBeacons` で非同期にサービスへ束縛します**。
その束縛が終わる前に投げた最初の領域の開始要求は取りこぼされ、
**スキャンが始まらないまま起動が終わります。**

`app.js` は起動直後に ranging を投げるので、そのままだと**コールドスタートで
測位できません**。閉じて開き直すと動くのは、前のセッションのスキャンが生きている
シングルトン（`BeaconManager` はアプリ単位）をそのまま使うからです。

**同じ領域をもう一度 `start` しても直りません。** 既に ranging 中の領域として
扱われ、サービスへの適用が素通りします。**`stop` を挟んで張り直してください。**
領域が一度外れるので、次の `start` が変更として適用されます。

`app.js` の `restartRanging` がこれをやっています。まだ1本も見ていない間だけ
3秒・8秒・20秒後に張り直し、1本でも見えたら止めます（館外で無駄に走らせない）。
実機では**起動から5秒ほどで測位に入ります**。

#### 切り分けを誤りやすい

最初は「権限ダイアログとの競合」と見て外しました（#191）。**許可済みで強制停止しても
再現する**という一言で崩れています。決め手はデバッグ表示で、
**デバッグビルドだけ強制停止後の初回でも測位できていた**ことでした。

差は `__DEBUG__` の有無だけで、そこで**「全UUID」の別の領域をもう1つ張っています**。
本来の領域はその適用に巻き込まれて走り出していた、というのが真相です。
`npm run copy:debug` で焼いたものと release で挙動が違ったら、この二重の ranging を疑ってください。
...
`app.getDiagnostics()` と、デバッグビルドの画面表示（`npm run copy:debug`）を使います。
「測位が出ないときの切り分け」の節を参照してください。ranging の回数と、
UUID を絞らない領域の受信数を並べて見るのが早いです。
